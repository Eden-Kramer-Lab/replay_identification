...
packages <- c("R.matlab", "here", "doSNOW", "multitaper", "mvtnorm", "splines")
...
place_bin_size <- 1  # bin size in space
### Read animal's linearized velocity place_bin_size Loren's group
position <- readMat(
  here("Raw-Data", "Bond", paste("bonlinpos0", day, ".mat", sep = "")))
...
n_time <- length(vel)  # n_time is the velocity data size

### Read animal's linearized trajecotry generated place_bin_size Ken, and clean them
data <- read.delim(
  here("Raw-Data", "Bond", paste("bond0", day, "0", epoch, ".dat", sep = "")),
       header = F, sep = " ")
...
yy <- (x[2:n_time])[idx]  # X_t
xx <- (x[1:(n_time - 1)])[idx]  # X_{t-1}
...
x_grid <- seq(min(x), max(x), place_bin_size = place_bin_size)
n_place_bins <- length(x_grid)

x_transition_0 <- matrix(0, n_place_bins, n_place_bins)
for (i in 1:n_place_bins) {
  x_transition_0[, i] <- (dnorm(x_grid, mean = x_grid[i], sd = sigma_500) /
                          sum(dnorm(x_grid, mean = x_grid[i], sd = sigma_500) *
                          place_bin_size))
}
x_transition_1 <- matrix(0, n_place_bins, n_place_bins)
for (i in 1:n_place_bins) {
  x_transition_1[, i] <- (dnorm(x_grid, mean = x_grid[i], sd = sigma_25) /
                          sum(dnorm(x_grid, mean = x_grid[i], sd = sigma_25) *
                          place_bin_size))
}

# Uniform distribution during transition state
x_transition_0to1 <- matrix(1 / round(max(x)) / place_bin_size, n_place_bins, n_place_bins)
...
n_time <- floor(diff(range(time_vel)) * freq) + 1
vel_1 <- approx(time_vel, vel, n = n_time)
time_vel_1 <- vel_1$x
vel_1 <- vel_1$y
summary(time_vel_1)
#### Perform interpolation for rat's trajectory, freq = 30Hz to 500Hz
x <- approx(time_vel, lin_pos, n = n_time)$y
...
N <- length(x_1)
is_ripple <- rep(0, n_time)
for (i in 1:N) {
  is_ripple[x1[i]:(x2[i])] <- 1
}
...
velocity_log_likelihood <- function(sigma, vel, n_time, mu) {
  log_p_v <- -log(sigma) - 0.5 * (vel[2:n_time] - mu) ^ 2 / sigma ^ 2
  return(log_p_v)
}

velocity_probability <- function(n_time, vel_1) {
  v_t_1 <- (vel_1[1:(n_time - 1)])  # v_{t-1}
  v_t <- vel_1[2:n_time]  # v_t
  sigma <- sd(v_t - v_t_1)
  mu <- vel_1[1:(n_time - 1)]
  mu[mu < 0] <- 0
  return(velocity_log_likelihood(sigma, vel, n_time, mu))
}

velocity_log_likelihood_ratio <- function(n_time, vel_1, is_ripple) {
  log_p_v_1 <- velocity_probability(n_time, vel_1[is_ripple == 1])
  log_p_v_0 <- velocity_probability(n_time, vel_1[is_ripple == 0])
  ### log_likelihood ratio
  log_likelihood_velocity <- log_p_v_1 - log_p_v_0
  log_likelihood_velocity[vel_1 > 4] <- -vel_1[vel_1 > 4]
  return(log_likelihood_velocity)
}

log_likelihood_velocity <- velocity_log_likelihood_ratio(n_time, vel_1, is_ripple)


## & 2.2 Fit p(I_t|I_t-1,v_t-1), replay state transition modeling

replay_state_transition <- function(is_ripple, n_time, vel_1, v, t, cm, s, i) {
  I_y <- is_ripple[-1]  # time t
  I_x <- is_ripple[-n_time]  # time t-1
  xx <- vel_1[-n_time]  # time t-1
  # logistic regression fit, with natural splines
  fit12 <- glm(I_y ~ I_x + bs(xx, knots = c(1, 2, 3, 20)),
               family = binomial(link = "logit"))
  # p(I_t|I_t-1,v_t-1) t=2:n_time
...
state_transition <- replay_state_transition(is_ripple, n_time, vel_1, v, t, cm, s, i)
...
idx <- ceiling( (range(time_vel_1) - c(starttime)) * LFP_SAMPLING_FREQUENCY) + 1
...
for (m in 1:dim(Y_f)[1]) {
  ...
  for (i in 1:dim(Y_f)[2]) {
    m1 <- spec.mtm(Y[(i - 1) * 30 + 1:30], nw = win_num, k = 2 * win_num - 1,
      nFFT = 150, log = "no", plot = F, deltat = 1 / LFP_SAMPLING_FREQUENCY)
    # nw is time-bandwidth, k is number of tapers set nFFT = 150, then place_bin_size=10 nFTT=30,
    # then place_bin_size=50. (freq # -1)*2 = nFTT
...
P_0 <- matrix(0, 6, sum(is_ripple == 0) + 1000)  # sum(is_ripple<1) = 45543
P_1 <- matrix(0, 6, sum(is_ripple == 1) + 500)  # sum(is_ripple>0) = 1656
p_0 <- rep(1, dim(P_0)[2])
p_1 <- rep(1, dim(P_1)[2])
x_2 <- c(starttime, x_2)
    ...
    if (N_1 > 0) {
      ...
      for (j in 1:(N_1 - 1)) {
        m1 <- spec.mtm(Y[(x_1[i] - starttime) * LFP_SAMPLING_FREQUENCY +
          (j - 1) * window_length + 1:window_length], nw = win_num, k = 2 *
          win_num - 1, nFFT = 150, log = "no", plot = FALSE,
          deltat = 1 / LFP_SAMPLING_FREQUENCY)
        # nFFT = 150, place_bin_size=10 nFTT=30, place_bin_size=50. (freq # -1)*2 = nFTT
        temp <- m1$spec[m1$freq == 200]
      ...
      win_num_1 <- floor(100 * (l_1 - (N_1 - 1) * window_length) / LFP_SAMPLING_FREQUENCY)
      m1 <- spec.mtm(Y[(x_1[i] - starttime) * LFP_SAMPLING_FREQUENCY + ((N_1 -
        1) * window_length + 1):l_1], nw = win_num_1, k = floor(2 * win_num_1 -
        1), nFFT = 150, log = "no", plot = F, deltat = 1 / LFP_SAMPLING_FREQUENCY)
      temp <- m1$spec[m1$freq == 200]
      # p_1=c(p_1, temp[!is.na(temp)] )
      p_1[k1] <- temp
      k1 <- k1 + 1
    ...
    if (N_2 > 0) {
      ...
      for (j in 1:(N_2 - 1)) {
        m1 <- spec.mtm(Y[(x_2[i] - starttime) * LFP_SAMPLING_FREQUENCY +
          (j - 1) * window_length + 1:window_length], nw = win_num, k = 2 *
          win_num - 1, nFFT = 150, log = "no", plot = F, deltat = 1 / LFP_SAMPLING_FREQUENCY)
        temp <- m1$spec[m1$freq == 200]
      ...
      win_num_1 <- floor(100 * (l_2 - (N_2 - 1) * window_length) / LFP_SAMPLING_FREQUENCY)
      m1 <- spec.mtm(Y[(x_2[i] - starttime) * LFP_SAMPLING_FREQUENCY + ( (N_2 -
        1) * window_length + 1):l_2], nw = win_num_1, k = floor(2 * win_num -
        1), nFFT = 150, log = "no", plot = F, deltat = 1 / LFP_SAMPLING_FREQUENCY)
      temp <- m1$spec[m1$freq == 200]
      # p_0=c(p_0, temp[!is.na(temp)] )
      p_0[k0] <- temp
...
H_1 <- (diag(sigma_1) * (4/(p + 2)/n_time)^(1/(p + 4)))^2
sigma_0 <- apply(P_0, 1, sd)
H_0 <- (diag(sigma_0) * (4/(p + 2)/n_time)^(1/(p + 4)))^2
...
log_likelihood_LFP <- log(f_1/f_0)
summary(log(f_1/f_0))
...
x_grid <- seq(-180, 180, place_bin_size = 1)
n_place_bins <- length(x_grid)
x_0 <- x[is_ripple == 0]

ptm <- proc.time()
denom_grid_0 <- numeric(n_place_bins)
for (i in 1:n_place_bins) {
...
registerDoSNOW(cl)  # Register back end Cores for Parallel Computing
denom_grid_1 <- 1/freq * foreach(i = 1:n_place_bins, .combine = "c") %dopar% {
...
for (tetrode in tetrode_idx) {
  ...
  # data path
  path1 <- here("Raw-Data", "Bond", "EEG", paste("bonmarks0", day, "-", idx, ".mat",
    sep = ""))
  data <- readMat(path1)
  ...
  ##########################################
  lambda0 <- numeric(n_time)
  idx <- is_ripple[(Spike_time - min(time)) * 500] == 0  # idx for non-replay state
  # non-ripple period
  ...
  Lambda_grid <- numeric(n_place_bins)

  stopCluster(cl)  # undo the parallel processing setup
  cl <- makeCluster(3, type = "SOCK")  # 3 - number of cores
  registerDoSNOW(cl)  # Register back end Cores for Parallel Computing

  num <- foreach(i = 1:n_place_bins, .combine = "c") %dopar% {
  ...
  idx <- c(1:n_time)[N > 0]  # idx for spike time
  ...
  num0 <- foreach(i = idx, .combine = "c", .packages = "mvtnorm") %dopar% {
    sum(dnorm(x_s_0, mean = x[i], sd = B_x) * dmvnorm(x = M_0, mean = M[sum(N[1:i]),
      ], sigma = B_m^2))
  }
  ...
  lambda1 <- numeric(n_time)

  idx <- is_ripple[(Spike_time - min(time)) * freq] == 1
  # idx for spiking during replay state
  Lambda1 <- freq * rep(sum(N[is_ripple > 0])/sum(is_ripple == 1), n_time)  # spikes per second
  ...
  idx <- c(1:n_time)[N > 0]  # idx for spike times
...
for (tetrode in tetrode_idx) {
  ...
  print(c(range(lambda0_all[k, ]), range(lambda1_all[k, ]), range(Lambda0_all[k,
    ]), range(Lambda1_all[k, ])))

  if (sum(is.na(c(range(lambda0_all[k, ]), range(lambda1_all[k, ]), range(Lambda0_all[k,
...
q_LFP <- rep(0, n_time)  # posterior probability given LFP and velocity
for (i in 2:n_time) {
  temp <- i%%10
  q1 <- (exp(log_likelihood_LFP[i%/%10 + 1] * (temp == 0)) * exp(log_likelihood_velocity[i - 1] * vel_idx) * (p_I_0[i -
    1] * (1 - q_LFP[i - 1]) + p_I_1[i - 1] * q_LFP[i - 1]))
  q2 <- ((1 - p_I_0[i - 1]) * (1 - q_LFP[i - 1]) + (1 - p_I_1[i - 1]) * q_LFP[i -
    1])
  q_LFP[i] <- q1/(q1 + q2)
}
summary(q_LFP)
...
vel_idx <- 0
q_remote <- numeric(n_time)
for (i in 2:n_time) {
  temp <- i%%10
  q1 <- (exp(log_likelihood_LFP[i%/%10 + 1] * (temp == 0)) * exp(log_likelihood_velocity[i - 1] * vel_idx) * L1[i] *
...
vel_idx <- 0
q_spiking <- numeric(n_time)
for (i in 2:n_time) {
  temp <- i%%10
  q1 <- exp(log_likelihood_velocity[i - 1] * vel_idx) * L1[i] * (p_I_0[i - 1] * (1 - q_spiking[i -
    1]) + p_I_1[i - 1] * q_spiking[i - 1])
  q2 <- ((1 - p_I_0[i - 1]) * (1 - q_spiking[i - 1]) + (1 - p_I_1[i - 1]) * q_spiking[i -
    1])
