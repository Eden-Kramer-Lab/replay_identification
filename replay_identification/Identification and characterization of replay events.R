...
# Decoding for rat Bond, day3, epoch2
...
## 1.1 AR(1) fit for animal's movement during active exploration; choose faster time rate to be 20~33 here.
...
data <- read.delim(paste('bond0',day,'0',epoch,'.dat',sep=''), header = F, sep=' ') 
lin_position = data[,1]*60; range(lin_position) # expand the position to range [-360, 360]
...
x <- as.numeric(lin_pos);range(x)  # x is rat's linearized position
...
alpha_30 = as.numeric(fit_position$coefficients); alpha_30

#### Transfer to new frequency rate, 500 Hz.
alpha_500 = (alpha_30^30)^(1/500); alpha_500   # freq = 500 Hz
sigma_500 = sqrt((1-alpha_500^2)*(1-alpha_30^(30*2))/(1-alpha_30^2)/(1-alpha_500^(500*2)))*sigma_30; sigma_500
# Model at faster time rate, 20 times
alpha_25 = (alpha_30^30)^(1/25); alpha_25   # 500Hz to 25Hz, 20 times faster
sigma_25 = sqrt((1-alpha_25^2)*(1-alpha_30^60)/(1-alpha_30^2)/(1-alpha_25^30))*sigma_30; sigma_25
sigma_25 = sqrt((1-alpha_25^2)/(1-alpha_30^2))*sigma_30; sigma_25 

#### Now define the state transition matrix
# (x_transition_0)_ij = p(x_t=x_i|x_t-1 = xj, I_t =0)
# (x_transition_1)_ij = p(x_t=x_i|x_t-1 = xj, I_t =1, I_t-1 =1)
x_grid = seq(min(x), max(x), by= by); m = length(x_grid)

x_transition_0 = matrix(0, m, m)
for (i in 1:m){
  x_transition_0[,i] = dnorm(x_grid, mean = x_grid[i], sd = sigma_500 )/
    sum(dnorm(x_grid, mean = x_grid[i], sd = sigma_500 )*by)
}
x_transition_1 = matrix(0, m, m)
for (i in 1:m){
  x_transition_1[,i] = dnorm(x_grid, mean = x_grid[i], sd = sigma_25 )/
    sum(dnorm(x_grid, mean = x_grid[i], sd = sigma_25 )*by)
}
x_transition_0to1 = matrix(1/round(max(x))/by, m, m) # Uniform distribution during transition state




## 1.2 read ad hoc replay state from ripple threshold method, consider the velocity threshold (4 cm/s)
...
vel_1 = vel_1$y;
time_vel_1 = vel_1$x; summary(time_vel_1)
...
idx=rep(FALSE,length(x_1)); # length=282
...
N=length(x_1);N  # N = 254
I=rep(0,n);  # n=floor(diff(range(time_vel))*freq)+1;n
for (i in 1:N){
  I[x1[i]:(x2[i])]=1
}
...
## 1.3 Read LFP data
### Read one channel to check detial
...
idx=c(ripple$tetlist);idx # idx=c(3,5,12,14,24,29) 6 tetrode
...
sigma1 = sd(y-x);sigma1; alpha1 =1  # random walk
mu_1 = vel_1[1:(n-1)]
mu_1[mu_1<0] = 0
log_p_v_1 = -log(sigma1) - .5*(vel_1[2:n]-mu_1)^2/sigma1^2  # log_likelihood

### Fit P(v_t|v_{t-1},I_t=1), out of replay state and no active exploration
idx=I[2:n]
x=(vel_1[1:(n-1)])[idx<1];
y=vel_1[2:n][idx<1];
idx = y<=4; x= x[idx]; y = y[idx]
sigma0 = sd(y-x); sigma0
mu_0 = vel_1[1:(n-1)]
log_p_v_0 = -log(sigma0) - .5*(vel_1[2:n]-mu_0)^2/sigma0^2

### log_likelihood ratio
l_vel= log_p_v_1-log_p_v_0 
time=time_vel_1
l_vel[vel_1>4] = -vel_1[vel_1>4]  
# Here the setting is a little artiticial,due to subjective understand about the data
...
plot(bins[-1], norm_spike, type='n', xlab = expression(v[t-1] (cm/s)), ylab="Ripple state prop" ,ylim = range(norm_spike)*1.2)
title( main = expression(I[t-1]==1))
for (i in 1:length(bins)){
  segments(bins[i],0, bins[i], norm_spike[i],
           col = 1, lwd=4)
}
...
fit12 <- glm (I_y ~ I_x+bs(xx,knots=c(1,2,3,20)), family=binomial(link='logit'))
...
plot(x_grid,y_0,type='l',col='red',xlim=c(0,6),main=expression(I[t-1]==0),
...
y_0 = exp(y_p$fit)/(1+exp(y_p$fit))
plot(x_grid, y_0,type='l',col='red',xlim=c(0,6), ylim=c(0,1.1), main=expression(I[t-1]==1),
     xlab=expression(v[t-1] (cm/s)),ylab=expression(p(I[t]==1)) )
y1 = exp(y_p$fit + 2* y_p$se.fit)/(1+exp(y_p$fit + 2*y_p$se.fit))
names(y_p)
lines(x_grid, y1, lty=2)
...
win_num=floor(100*window_length/1500); win_num # calculate the window number
...
for (m in 1:dim(Y_f)[1]){
  ...
  for (i in 1:dim(Y_f)[2]) {
    ...
    # nw is time-bandwidth, k is number of tapers
    # set nFFT = 150, then by=10; nFTT=30, then by=50. (freq # -1)*2 = nFTT
...
p_0=rep(1,dim(P_0)[2]); p_1 =rep(1,dim(P_1)[2])
x_2 = c(starttime, x_2)
...
for (m in 1:dim(y)[1]){
  ...
  for (i in 1:length(x_1)) {
    ...
    if (N_1>0){
      ...
      for (j in 1:(N_1-1)){
        m1=spec.mtm(Y[(x_1[i]-starttime)*1500+(j-1)*window_length+1:window_length],
                    nw=win_num, k=2*win_num-1,nFFT=150, log='no',
                    plot=FALSE,deltat=1/1500)
      ...
      m1=spec.mtm(Y[(x_1[i]-starttime)*1500+((N_1-1)*window_length+1):l_1],
                  nw=win_num_1, k=floor(2*win_num_1-1),nFFT=150, 
    ...
    if (N_2>0){
      ...
      for (j in 1:(N_2-1)){
        m1=spec.mtm(Y[(x_2[i]-starttime)*1500+(j-1)*window_length+1:window_length],
      ...
      m1=spec.mtm(Y[(x_2[i]-starttime)*1500+((N_2-1)*window_length+1):l_2],
                  nw=win_num_1, k=floor(2*win_num-1),nFFT=150,
                  log='no',plot=F,deltat=1/1500)
      temp=m1$spec[m1$freq<=200 & m1$freq>=200]
      #p_0=c(p_0, temp[!is.na(temp)] )  
      p_0[k0]=temp; k0=k0+1
...
#save(P_0,P_1, file =paste(path,'Data/spectral power in 2 states.RData',sep=''))
...
summary(log(f_1/f_0))
#save(f_1,f_0, file= paste(path,'Data/nonparametric spectral regression day3_epoch6.RData',sep=''))
...
B_x = b_x = 360*.15*10e-2; b_x  # 1.5% of linear track length
B_m = diag(rep(20,4)); B_m  
T = diff(range(time)); T # duration of the experiment 
...
tetrode_idx = c(1:5,7:8,10:14,17:25,27:29)  # tetrode index for Bond, day3, epoch2

x_grid = seq(-180,180, by =1)
m = length(x_grid); m
x_0 = x[I<1]; length(x_0)
...
for (tetrode in tetrode_idx){  
  ...
  if (tetrode >=10){idx = paste(tetrode, sep='')}
  
  
  path1 = paste('C:/BostonData/bond03/raw_spikes/bond0',day,
                '-',idx,'_params.mat',sep='') # data path
  ...
  M = data$filedata[,,1]$params[idx,2:5]; dim(M) # mark space: M, dim = N * 4
  rm(data)
  
  N = numeric(n)
  temp = (Spike_time - min(time) )*500
  for(i in temp){ N[i] = N[i]+1 }
  ...
  ##########################################
  lambda0 = numeric(n)
  idx = I[(Spike_time - min(time))*500]<1; sum(idx)  # idx for non-replay state
  x_s_0 = approx(time_vel, lin_pos, xout = Spike_time[idx])$y; length(x_s_0) # non-ripple period
  ...
  Lambda0 = Lambda_grid[round(x) - min(x_grid) +1]  # Marginal intensity rate
  ...
  idx = I[(Spike_time - min(time))*freq]>0; sum(idx)  
  ...
  # x_s_1 is the animal's position at all spike moment during replay
  #M_1 is the mark value at all spike moment during replay
...
#load(paste(path,'Data/unsorted spiking kernel fit Bond day3 epoch',epoch,' homogeneous.RData',sep=''))
...
for (tetrode in tetrode_idx){  
  ...
  for (i in temp){
    N[i] = N[i]+1 }
  print(c(range(lambda0_all[k,]),range(lambda1_all[k,]),
          range(Lambda0_all[k,]),range(Lambda1_all[k,])) )
  
  if (sum(is.na(c(range(lambda0_all[k,]),range(lambda1_all[k,]),
                  range(Lambda0_all[k,]),range(Lambda1_all[k,]))))==0 ){
    log_p_0 =log_p_0+(-Lambda0_all[k,]*dt + N*log(lambda0_all[k,]*dt + 1e-200) ); 
    log_p_1 =log_p_1+(-Lambda1_all[k,]*dt + N*log(lambda1_all[k,]*dt + 1e-200) );
  }
  k = k+1
}
...
#################################################
### LFP and velocity only
vel_idx = 0 # whether we consider p(v_t|v_{t-1}, I_t)
q_LFP=rep(0,n) # posterior probability given LFP and velocity
for (i in 2:n) {
  temp = i%%10
  q1=exp(l_LFP[i%/%10+1]*(temp==0))*exp(l_vel[i-1]*vel_idx)*(p_I_0[i-1]*(1-q_LFP[i-1])+p_I_1[i-1]*q_LFP[i-1])
  q2=((1-p_I_0[i-1])*(1-q_LFP[i-1])+(1-p_I_1[i-1])*q_LFP[i-1])
  q_LFP[i]=q1/(q1+q2)
}
summary(q_LFP)
...
for (i in 2:n) {
  ...
  q2=((1-p_I_0[i-1])*(1-q_remote[i-1])+(1-p_I_1[i-1])*q_remote[i-1])
  q_remote[i]=q1/(q1+q2)
}
summary(q_remote)
...
for (i in 2:n) {
  temp = i%%10
  q1=exp(l_vel[i-1]*vel_idx)*L1[i]*
    (p_I_0[i-1]*(1-q_spiking[i-1])+p_I_1[i-1]*q_spiking[i-1])
  q2=((1-p_I_0[i-1])*(1-q_spiking[i-1])+(1-p_I_1[i-1])*q_spiking[i-1])
