...
data <- read.delim(paste("bond0", day, "0", epoch, ".dat", sep = ""), header = F,
    sep = " ")
...
for (i in 1:m) {
    x_transition_0[, i] <- dnorm(x_grid, mean = x_grid[i], sd = sigma_500) / sum(dnorm(x_grid,
        mean = x_grid[i], sd = sigma_500) * by)
}
x_transition_1 <- matrix(0, m, m)
for (i in 1:m) {
    x_transition_1[, i] <- dnorm(x_grid, mean = x_grid[i], sd = sigma_25) / sum(dnorm(x_grid,
        mean = x_grid[i], sd = sigma_25) * by)
}
...
for (tetrode in tetrode_idx) {
    ...
    path1 <- paste("C:/BostonData/bond03/raw_spikes/bond0", day, "-", idx, "_params.mat",
        sep = "")  # data path
    ...
    x_s_0 <- approx(time_vel, lin_pos, xout = Spike_time[idx])$y  # non-ripple period
...
for (i in 2:n) {
    temp <- i %% 10
    q1 <- exp(l_LFP[i %/% 10 + 1] * (temp == 0)) * exp(l_vel[i - 1] * vel_idx) * (p_I_0[i -
...
for (i in 2:n) {
    temp <- i %% 10
    q1 <- exp(l_vel[i - 1] * vel_idx) * L1[i] * (p_I_0[i - 1] * (1 - q_spiking[i -
